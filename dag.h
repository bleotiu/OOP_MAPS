...
#ifndef TEMA_3_POO_HARTA_DAG_H
#define TEMA_3_POO_HARTA_DAG_H
...
int DirectedAcyclicGraph :: size() {
    return n;
}

double DirectedAcyclicGraph :: Dijkstra(int start, int finish) {
    --start; --finish;
...
void DirectedAcyclicGraph :: AddEdge(const int x, const int y) {
    double D = DistantaPlan (nodes[x - 1],nodes[y - 1]);
    edges[x - 1].push_back (make_pair(y - 1,D));
}

void DirectedAcyclicGraph :: GetDistance (const int x, const int y){
